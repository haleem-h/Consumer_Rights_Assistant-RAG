...
# -------------------------------
st.set_page_config(
    page_title="Consumer Rights Assistant",
    page_icon="🤖",
    layout="centered"
)

# -------------------------------
# BLACK + SILVER UI (UNCHANGED)
# -------------------------------
st.markdown("""
<style>
...
.user { background: linear-gradient(145deg, #2c2c2c, #1a1a1a); color: white; }
.bot { background: linear-gradient(145deg, #cfd0d0, #8a8b8d); color: black; }
...
if query:
    ...
    if not docs:
        st.markdown(
            "<div class='chat-bubble bot'>🤖 Information not found in documents.</div>",
            unsafe_allow_html=True
        )
    else:
        ...
        prompt = f"""
You are a Consumer Rights Assistant in India.
...
"""
        ...
        st.markdown(
            f"<div class='chat-bubble bot'>🤖 {answer}</div>",
            unsafe_allow_html=True
        )
